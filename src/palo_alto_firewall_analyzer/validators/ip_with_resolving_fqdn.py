            ...
            if ip in ips and ips_fqdns_resolve_to[ip] == 1:                
                for address_name, ipnetmask_value, address_entry in ips[ip]:
                    ...
                    detail={
                        ...
                        "entry_name":address_name,
                        "fqdn":fqdn,
                        "ip":ipnetmask_value
                    }
                    badentries.append(BadEntry(data=(address_entry, fqdn), text=text, device_group=device_group, entry_type='Addresses',Detail=parsed_details(detail)))
            count_checks+=1        
