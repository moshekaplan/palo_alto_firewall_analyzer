...
from palo_alto_firewall_analyzer.core import BadEntry, register_policy_fixer, get_policy_validators, xml_object_to_dict
...
def consolidate_service_like_objects(profilepackage, object_type, object_friendly_type, validator_function):
    ...
    for device_group, objects_to_consolidate in dg_to_objects_to_consolidate.items():
        ...
        print(f"Replacing the contents of {len(servicegroups_needing_replacement)} Service Groups" )
        for object_dg, object_type, object_entry in servicegroups_needing_replacement:
            ...
            text = f"The following services were replaced in {object_dg}'s {object_type} {object_entry.get('name')}: {sorted([k + ' with ' + v for k, v in replacements_made.items()])}"
            badentries.append(BadEntry(data=[object_entry], text=text, device_group=object_dg, entry_type=object_type))
            if object_dg == 'shared':
                pan_api.update_devicegroup_object(panorama, version, api_key, object_policy_dict, object_type, 'shared')
            else:
                pan_api.update_devicegroup_object(panorama, version, api_key, object_policy_dict, object_type, 'device-group', object_dg)

        # Then replace the contents of policies
        print(f"Replacing the contents of {len(policies_needing_replacement)} Policies" )
        for policy_dg, policy_type, policy_entry in policies_needing_replacement[:25]:
            ...
            text = f"The following services were replaced in {policy_dg}'s {policy_type} {policy_entry.get('name')}: {sorted([k + ' with ' + v for k, v in replacements_made.items()])}"
            badentries.append(BadEntry(data=[object_entry], text=text, device_group=object_dg, entry_type=object_type))
            if policy_dg == 'shared':
                pan_api.update_devicegroup_policy(panorama, version, api_key, object_policy_dict, policy_type, 'shared')
            else:
                pan_api.update_devicegroup_policy(panorama, version, api_key, object_policy_dict, policy_type, 'device-group', policy_dg)
